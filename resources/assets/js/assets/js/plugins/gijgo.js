  ...
  init: function(jsConfig) {
    ...
    $handleEl.on("touchstart mousedown", function(e) {
      ...
      gj.documentManager.subscribeForEvent(
        "touchmove",
        $dragEl.data("guid"),
        gj.draggable.methods.createMoveHandler($dragEl)
      );
      gj.documentManager.subscribeForEvent(
        "mousemove",
        $dragEl.data("guid"),
    ...
    return function(e) {
      ...
      if ($dragEl.attr("data-draggable-dragging") === "true") {
        ...
        if (prevX && prevY) {
          ...
          if (
            false !== gj.draggable.events.drag($dragEl, offsetX, offsetY, x, y)
          ) {
...
gj.droppable.methods = {
  ...
  isOver: function($dropEl, mousePosition) {
    ...
    return (
      mousePosition.left > offsetLeft &&
      mousePosition.left < offsetLeft + $dropEl.outerWidth(true) &&
      mousePosition.top > offsetTop &&
      mousePosition.top < offsetTop + $dropEl.outerHeight(true)
    );
  },
...
gj.grid.plugins.expandCollapseRows = {
  ...
  private: {
    detailExpand: function($grid, $cell) {
      ...
      gj.grid.plugins.expandCollapseRows.events.detailExpand(
        $grid,
        $detailsRow.find("td>div"),
        id
      );
    },
    ...
    detailCollapse: function($grid, $cell) {
      ...
      gj.grid.plugins.expandCollapseRows.events.detailCollapse(
        $grid,
        $detailsRow.find("td>div"),
        id
      );
  ...
  public: {
    ...
    collapseAll: function() {
      ...
      $grid.find('tbody tr[data-role="row"]').each(function() {
        gj.grid.plugins.expandCollapseRows.private.detailCollapse(
          $grid,
    ...
    expandAll: function() {
      var $grid = this,
        position = gj.grid.methods.getColumnPositionByRole($grid, "expander");
      $grid.find('tbody tr[data-role="row"]').each(function() {
        gj.grid.plugins.expandCollapseRows.private.detailExpand(
          $grid,
          $(this).find("td:eq(" + position + ")")
        );
      });
  ...
  renderers: {
    editManager: function(value, record, $cell, $displayEl, id, $grid) {
      var data = $grid.data(),
        ...
        $update = $(data.inlineEditing.updateButton)
          .attr("data-key", id)
          .hide(),
        $cancel = $(data.inlineEditing.cancelButton)
          .attr("data-key", id)
          .hide();
  ...
  localization: function(data) {
    if (data.uiLibrary === "bootstrap") {
      data.inlineEditing.editButton =
        '<button type="button" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> ' +
        gj.grid.messages[data.locale].Edit +
        "</button>";
      data.inlineEditing.deleteButton =
        '<button type="button" class="btn btn-default btn-sm gj-margin-left-10"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span> ' +
        gj.grid.messages[data.locale].Delete +
        "</button>";
      data.inlineEditing.updateButton =
        '<button type="button" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> ' +
        gj.grid.messages[data.locale].Update +
        "</button>";
      data.inlineEditing.cancelButton =
        '<button type="button" class="btn btn-default btn-sm gj-margin-left-10"><span class="glyphicon glyphicon-ban-circle" aria-hidden="true"></span> ' +
        gj.grid.messages[data.locale].Cancel +
        "</button>";
    } else {
      data.inlineEditing.editButton =
        '<button class="gj-button-md"><i class="gj-icon pencil" /> ' +
        gj.grid.messages[data.locale].Edit.toUpperCase() +
        "</button>";
      data.inlineEditing.deleteButton =
        '<button class="gj-button-md"><i class="gj-icon delete" /> ' +
        gj.grid.messages[data.locale].Delete.toUpperCase() +
        "</button>";
      data.inlineEditing.updateButton =
        '<button class="gj-button-md"><i class="gj-icon check-circle" /> ' +
        gj.grid.messages[data.locale].Update.toUpperCase() +
        "</button>";
      data.inlineEditing.cancelButton =
        '<button class="gj-button-md"><i class="gj-icon cancel" /> ' +
      ...
      if ($editorField.length) {
        ...
        if (typeof column.editor === "function") {
          ...
          if ("checkbox" === column.type && gj.checkbox) {
            ...
            $editorField = $(
              '<input type="text" value="' + value + '" class="gj-width-full"/>'
            );
...
gj.tree.methods = {
  ...
  getRecords: function($tree, response) {
    ...
    for (i = 0; i < response.length; i++) {
      ...
      if (
        response[i][data.childrenField] &&
        response[i][data.childrenField].length
      ) {
        nodeData.children = gj.tree.methods.getRecords(
          $tree,
          response[i][data.childrenField]
        );
  ...
  appendNode: function($tree, $parent, nodeData, level, position) {
    var i,
      ...
      data = $tree.data(),
      $node = $(
        '<li data-id="' + nodeData.id + '" data-role="node" />'
      ...
      $display = $(
        '<span data-role="display">' + nodeData.data[data.textField] + "</span>"
      ),
      hasChildren =
        typeof nodeData.data[data.hasChildrenField] !== "undefined" &&
        nodeData.data[data.hasChildrenField].toString().toLowerCase() ===
          "true",
      disabled =
        typeof nodeData.data[data.disabledField] !== "undefined" &&
        nodeData.data[data.disabledField].toString().toLowerCase() === "true";
    ...
    if (data.imageCssClassField && nodeData.data[data.imageCssClassField]) {
      $(
        '<span data-role="image"><span class="' +
          nodeData.data[data.imageCssClassField] +
          '"></span></span>'
      ...
      $span = $(
        '<span data-role="image">' +
          nodeData.data[data.imageHtmlField] +
          "</span>"
      );
      $span.insertBefore($display);
    }
  ...
  addNode: function($tree, data, $parent, position) {
    ...
    if (!$parent || !$parent.length) {
      ...
      gj.tree.methods
        .getById($tree, $parent.parent().data("id"), $tree.data("records"))
  ...
  private: {
    nodeDataBound: function($tree, $node) {
      ...
      if ($wrapper.length && $display.length) {
        $display.on(
          "mousedown",
          gj.tree.plugins.dragAndDrop.private.createNodeMouseDownHandler(
            $tree,
            $node,
            $display
          )
...
gj.datepicker.methods = {
  ...
  initialize: function($datepicker) {
    ...
    if (data.keyboardNavigation) {
      $datepicker.on(
        "keydown",
        gj.datepicker.methods.createKeyDownHandler($datepicker, $calendar)
      );
    }
  },
  ...
  createCalendar: function($datepicker) {
    ...
    $row.append(
      $("<th><div>" + data.icons.previousMonth + "</div></th>").on(
        "click",
    ...
    $row.append(
      '<th colspan="' +
        (data.calendarWeeks ? 6 : 5) +
        '"><div role="month"></div></th>'
    );
    $row.append(
      $("<th><div>" + data.icons.nextMonth + "</div></th>").on(
        "click",
    ...
    for (
      i = data.weekStartDay;
      i < gj.datepicker.messages[data.locale].weekDays.length;
      i++
    ) {
      $row.append(
        "<th><div>" +
          gj.datepicker.messages[data.locale].weekDays[i] +
          "</div></th>"
      );
    }
  ...
  renderCalendar: function($datepicker) {
    ...
    $table
      .find('thead [role="month"]')
      .text(gj.core.messages[data.locale].monthNames[month] + " " + year);
    ...
    for (i = 1; i <= firstDayPosition; i++) {
      ...
      if (
        prevMonth.year === selectedDay.getFullYear() &&
        prevMonth.month === selectedDay.getMonth() &&
    ...
    for (i = 1; weekDay != 0; i++) {
      ...
      if (
        nextMonth.year === selectedDay.getFullYear() &&
        nextMonth.month === selectedDay.getMonth() &&
        i === selectedDay.getDate()
      ) {
        $cell = $('<td class="selected" />');
      } else {
  ...
  show: function($datepicker) {
    ...
    $calendar
      ...
      .css("top", offset.top + $datepicker.outerHeight(true) + 3);
    $calendar.show();
    $datepicker.focus();
...
gj.timepicker.methods = {
  ...
  initialize: function($timepicker) {
    var data = $timepicker.data(),
      ...
      $rightIcon =
        data.uiLibrary !== "materialdesign" &&
        data.iconsLibrary === "materialicons"
          ? $(
    ...
    $rightIcon.on("click", function(e) {
      var $clock = $("body").find(
        '[role="clock"][guid="' + $timepicker.attr("data-guid") + '"]'
      );
      if ($clock.is(":visible")) {
    ...
    if (data.keyboardNavigation) {
      $timepicker.on(
        "keydown",
        gj.timepicker.methods.createKeyDownHandler($timepicker, $calendar)
      );
    }
  },
  ...
  createClock: function($timepicker) {
    ...
    $dial.on(
      "mousedown",
      gj.timepicker.methods.mouseDownHandler($timepicker, $clock)
    );
    $dial.on(
      "mousemove",
      gj.timepicker.methods.mouseMoveHandler($timepicker, $clock)
    );
    $dial.on(
      "mouseup",
      gj.timepicker.methods.mouseUpHandler($timepicker, $clock)
    );
  ...
  renderHours: function($timepicker, $clock) {
    ...
    $dial.append(
      '<div role="arrow" style="transform: rotate(-90deg); display: none;"><div class="c296"></div><div class="c297"></div></div>'
    );
  ...
  renderMinutes: function($timepicker, $clock) {
    ...
    $dial.append(
      '<span role="hour" style="transform: translate(-1.98393e-14px, -108px);">60</span>'
    );
  ...
  show: function($timepicker) {
    ...
    if (!$timepicker.value()) {
      $timepicker.value(
        gj.core.formatDate(new Date(), data.format, data.locale)
      );
    }
    $clock.show();
    ...
    if (data.modal) {
      ...
      $clock
        .css("left", offset.left)
        .css("top", offset.top + $timepicker.outerHeight(true) + 3);
    }
    $timepicker.focus();
